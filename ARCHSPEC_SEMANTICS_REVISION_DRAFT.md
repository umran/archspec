...
11. **External effect result replay** — *Resolved 2026-09-05 by `CONSEQO_REVISION_V3_AMENDMENT_B_EXTERNAL_IDEMPOTENCY_RETRYABLE_ERRORS.md`; exposed 2026-09-04 (V3 §31, §48.2, §48.6).*  
    As originally posed: an `ExternalEffect` could declare a `result: Result<Ok, Err>` contract, but no declared fact made an external boundary's returned result replay-consistent, so every program branching on a provider's answer was unproven for idempotency and result replay, however the boundary actually behaved. The design questions were whether the discharging guarantee should be a separate declaration, scoped to key or instance equality, and whether a partial guarantee deserved vocabulary.  
    Amendment B answered them by **strengthening `deduplicated_by` itself**, scoped to key equality alone, with no separate declaration and no partial guarantee: for a result-bearing external effect, equal evaluated keys identify one logical external interaction whose *terminal* result the guarantee fixes — same variant, replay-equivalent payload, once terminally resolved. The boundary that answers a duplicate with a distinct "already processed" error, which motivated the original caution, is handled by conformance rather than vocabulary: such a boundary does not satisfy the strengthened guarantee unless it abstracts the duplicate response back into the original logical result. The attempt/terminal distinction the strengthening needs came with it: `ResultType.err` now declares a disposition (`unspecified` | `terminal` | `retryable`), so a retryable error is an attempt-level, nonterminal outcome the guarantee never promotes, and `unspecified` keeps its epistemic meaning. `Ok` is terminal by definition. Both fixture gaps closed or sharpened: `transcode_video` proves (terminal `RenderFailed`), `charge_payment` remains honestly unproven (`not_deduplicated` card charge). What Amendment B deliberately did **not** introduce — heterogeneous per-error-class dispositions inside one contract, and any retry-execution policy that would consume `retryable` — remains open for a later revision.
